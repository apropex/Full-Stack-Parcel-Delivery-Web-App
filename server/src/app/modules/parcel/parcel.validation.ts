...
export const createParcelZodSchema = z.object({
  ...
  description: z
    .string({
      error: ({ input }) =>
        input === undefined ? "Description is required" : "Description must be a string",
    })
    .max(1000, { error: "Description cannot exceed 1000 characters" })
    .trim(),

  type: z.enum(Object.values(eParcelTypes), {
  ...
  weight: z
    .number({
      error: ({ input }) =>
        input === undefined ? "Weight is required" : "Weight must be a number",
    })
