...
import { generateAccessTokenByRefreshToken, generateAllTokens } from "../../lib/jwt";
...
export const getNewAccessTokenController = catchAsync(async (req, res) => {
  ...
  if (!refreshToken) throw new AppError(sCode.BAD_REQUEST, "Refresh token not found");
  ...
  sendResponse(res, {
    statusCode: sCode.OK,
    message: "Set new access token successfully",
    // data: token,
  });
});
